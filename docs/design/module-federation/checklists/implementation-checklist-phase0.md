...
- [ ] Create `src/app-api/types/AppContext.ts` — `AppContext`, `CyAppWithLifecycle` (facade API fields commented out until each phase)
...
- [ ] Create `src/app-api/index.ts` — top-level barrel export (facade hooks commented out initially)
