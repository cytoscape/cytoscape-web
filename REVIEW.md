...
> **Round-3 status:** items 1–4 above plus the auth-token leak (R2-11) are now **FIXED**, each proven by a regression test that was written first and shown to fail against the old code. R2-2 is fixed for the four Immer-managed stores via a shared `persistNetworkSlices` middleware; **NetworkStore still has the defect** (its cy-backed networks mutate in place, so slice references never change — it needs per-action persistence; see the R2-2 status note). Details in [What was done — Round 3](#round-3-test-driven-fixes-for-the-p0s).
...
**Residual:** **NetworkStore keeps the old wrapper and the defect.** Its slices are cy-backed `Network` objects mutated in place, so identity diffing cannot detect changes; it needs per-action persistence (actions already receive the networkId) or an explicit dirty-marking mechanism. **WorkspaceStore** (singleton, reference-diffed) was untouched. Analysis refinements from round 3: `TableStore.add`/`ViewModelStore.add`/`VisualStyleStore.add` *did* explicitly persist with the correct id at creation time (which is why non-current networks existed in the DB at all) — the defect applied to every mutation *after* creation.
...
| **P0** | NetworkStore persistence keying (R2-2 residual): per-action persistence or dirty-marking — identity diffing can't see in-place cy mutation. | M |
...
| **P2** | `as const` on `VisualPropertyValueTypeName` + fix `valueType2BaseType`; add `tsc --noEmit` to `npm test` (R2-22, A8 — tsc is at 0 errors as of round 3, so the gate is now free). | S |
