...
;(async function exportDatabaseSnapshot() {
  try {
    ...
    for (const storeName of objectStoreNames) {
      try {
        ...
        if (!db.objectStoreNames.contains(storeName)) {
          console.warn(
            `⚠️ Object store "${storeName}" does not exist, skipping`,
          )
          data[storeName] = []
          continue
        }
    ...
    console.log(
      `📁 Filename: cyweb-db-snapshot-${exportVersion}-${dateStr}.json`,
    )
    console.log(`📊 Total records: ${totalRecords}`)
    ...
    if (error.name === 'VersionError') {
      ...
      console.error(
        '   indexedDB.open("cyweb-db").onsuccess = (e) => console.log("Version:", e.target.result.version);',
      )
    }
