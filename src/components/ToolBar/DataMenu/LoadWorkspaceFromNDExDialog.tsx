...
export const LoadWorkspaceFromNDExDialog = (
  ...
  const emptyListMessage =
    currentTabIndex === 0
      ? 'No search results'
      : 'No workspaces found in your NDEx account'

  const myNetworksTab = authenticated ? (
    <Tab label={<Typography>My Workspaces</Typography>}></Tab>
  ) : (
    <Tooltip title="Login to NDEx to access your networks">
      <Box>
        <Tab disabled label={<Typography>My Workspaces</Typography>}></Tab>
      </Box>
    </Tooltip>
  )
  ...
  const fetchSearchResults = async (searchValue: string): Promise<void> => {
    ...
    const workspaceUser = await ndexClient.getSignedInUser()
    console.log(workspaceUser.externalId)
  ...
  return (
    <Dialog
      ...
      onClose={handleClose}
    >
      <DialogTitle>NDEx - Workspace Browser</DialogTitle>
      <DialogContent>
