...
/**
 * Help > Citation. Owned by `HelpMenu`, not by the menu row, so it outlives
 * the menu that opened it (see `LicenseDialog`).
 */
export const CitationDialog = ({
  open,
  onClose,
}: CitationDialogProps): ReactElement => {
  ...
  return (
    <CyDialog data-testid="citation-dialog" open={open} maxWidth="md" fullWidth>
      <DialogTitle>
        <Box
          sx={{
            display: 'flex',
      ...
      <DialogActions sx={{ m: 0, padding: (theme) => theme.spacing(2, 3) }}>
        <Button
          data-testid="citation-dialog-close-button"
          variant="contained"
          onClick={onClose}
        >
          Close
