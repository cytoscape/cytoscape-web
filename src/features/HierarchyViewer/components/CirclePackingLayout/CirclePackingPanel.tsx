...
export const CirclePackingPanel = ({
  ...
  const zoomBehaviorRef = useRef<d3Zoom.ZoomBehavior<
    SVGSVGElement,
    unknown
  > | null>(null) // Keep the zoom behavior as a state
  ...
  const updateVisibilityForZoom = (maxDepth: number): void => {
    const circles = d3Selection.selectAll<
      SVGCircleElement,
      d3Hierarchy.HierarchyCircularNode<D3TreeNode>
    >('circle')

    circles.style('display', (d): string => {
    ...
    const textElements = d3Selection.selectAll<
      SVGTextElement,
      d3Hierarchy.HierarchyCircularNode<D3TreeNode>
    >('text')

    textElements.style('display', (d): string => {
      // Guard against undefined data
      if (!d || d === undefined) {
        return 'none'
      }
  ...
  const drawCirclePacking = (
    rootNode: d3Hierarchy.HierarchyCircularNode<D3TreeNode>,
  ): void => {
    // Pick the base tag
    const svgElement = svgRef.current
    if (svgElement === null) return
    const svg = d3Selection.select<SVGSVGElement, unknown>(svgElement)
    ...
    wrapper
      .append('g')
      .selectAll<SVGTextElement, d3Hierarchy.HierarchyCircularNode<D3TreeNode>>(
        'text',
      )
      .data(rootNode.descendants())
      .join('text')
  ...
  useEffect(
    function onCirclePackingViewChange() {
      ...
      if (networkId !== lastNetworkId || !wrapperExists) {
        ...
        const existingCirclesGroup =
          wrapper.select<SVGGElement>('g:first-child')
        const circlesGroup = existingCirclesGroup.empty()
          ? wrapper.append('g')
          : existingCirclesGroup
        ...
        const existingTextGroup = wrapper.select<SVGGElement>('g:nth-child(2)')
        const textGroup = existingTextGroup.empty()
          ? wrapper.append('g')
          : existingTextGroup
