...
describe('installGate', () => {
  describe('parseSingleEntryManifest', () => {
    ...
    it('returns the first entry and warns when more than one is present', () => {
      const warnSpy = jest.spyOn(logApp, 'warn').mockImplementation(() => {})
  ...
  describe('isHostCompatible', () => {
    ...
    it('returns true when the host version is unknown', () => {
      expect(isHostCompatible('>=2.0.0', undefined)).toBe(true)
    })
  })
})
