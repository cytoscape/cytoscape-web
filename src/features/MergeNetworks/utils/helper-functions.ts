...
export const getNetTableFromSummary = (summary: NetworkSummary): Table => {
  ...
  summary.properties.forEach((property: NetworkProperty) => {
    cols.push({
      name: property.predicateString,
      type: property.dataType,
    } as Column)
    tableRows.push([
      `${tableRows.length}`,
      { [property.predicateString]: property.value },
    ])
  })
...
export function filterRows(rows: MatchingTableRow[]) {
  return rows.filter((row) => {
    const allNone = Object.keys(row.nameRecord).every(
      (key) => row.nameRecord[key] === 'None',
    )
    return !allNone
  })
}

export function getMergedType(
...
export function sortListAlphabetically(
  list: [string, IdType][],
): [string, IdType][] {
