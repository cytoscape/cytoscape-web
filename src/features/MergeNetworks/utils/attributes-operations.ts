...
export function checkAttribute(
  nodeMatchingTable: MatchingTable,
  edgeMatchingTable: MatchingTable,
      ...
      if (
        ...
        (row.nameRecord[netId] !== 'None' &&
          !networkRecords[netId]?.nodeTable?.columns.some(
            (col) =>
              col.name === row.nameRecord[netId] &&
              col.type === row.typeRecord[netId],
          ))
      ) {
        return true
      }
...
export function castAttributes(
  toMergeAttr: Record<string, ValueType> | undefined,
  netId: IdType,
  ...
  if (toMergeAttr !== undefined) {
    for (const row of isNode
      ...
      if (
        ...
        if (
          isString(val) &&
          ['null', 'nan', 'none'].includes(val.toLowerCase())
        ) {
...
export function addMergedAtt(
  castedRecord: Record<string, ValueType>,
  oriMatchingVal: ValueType | undefined,
  mergedAttCol: Column,
): Record<string, ValueType> {
  ...
  castedRecord[mergedAttCol.name] = typeCoercion(
    oriMatchingVal,
    mergedAttCol.type,
  )
  return castedRecord
...
export function attributeValueMatcher(
  val: ValueType,
  nodeAttMap: Map<SingleValueType, IdType>,
): string {
  if (val !== undefined) {
...
function listValueTypeCoercion(
  val: ValueType,
  mergedType: ValueTypeName,
): ListOfValueType {
  ...
  switch (singleType) {
    case ValueTypeName.String:
      return valArray.map((v) =>
        singleValueTypeCoercion(v, singleType),
      ) as string[]
...
function singleValueTypeCoercion(
  val: ValueType,
  mergedType: ValueTypeName,
): SingleValueType {
  if (isSingleType(mergedType)) {
    ...
    throw new Error(
      `Cannot convert the attribute value to the type ${mergedType}`,
    )
  }
}
...
export function isConvertible(
  typeFrom: ValueTypeName,
  typeTo: ValueTypeName,
): boolean {
  if (typeFrom === typeTo) {
    return true
  }
  ...
  if (
    typeFrom === ValueTypeName.Integer &&
    (typeTo === ValueTypeName.Long || typeTo === ValueTypeName.Double)
  ) {
    return true
  }
  ...
  if (
    isListType(typeTo) &&
    isConvertible(getPlainType(typeFrom), getPlainType(typeTo))
  ) {
    return true
  }
  return false
...
export function getAllConvertiableTypes(
  types: Set<ValueTypeName | 'None'>,
): ValueTypeName[] {
  const singleTypes = [
  ...
  const convertiableListTypes = convertiableSingleTypes.map(
    (type) => `list_of_${type}` as ValueTypeName,
  )
...
export function mergeAttributes(
  orinalRow: Record<string, ValueType>,
  castedRecord: Record<string, ValueType>,
): Record<string, ValueType> {
  ...
  Object.entries(castedRecord).forEach(([key, value]) => {
    if (!mergedRow.hasOwnProperty(key)) {
      ...
      if (
        (mergedRow[key] as ListOfValueType).every(
          (item) => typeof item === typeof value[0],
        )
      ) {
        mergedRow[key] = [
          ...new Set([...(mergedRow[key] as ListOfValueType), ...value]),
        ] as ValueType
      } else {
        throw new Error(
          `Type mismatch for key ${key}: ${typeof (mergedRow[key] as ListOfValueType)[0]} vs ${typeof value[0]}`,
        )
      }
    }
  })
...
export function duplicateAttName(mergedAttributes: Column[]): boolean {
  return (
    new Set(mergedAttributes.map((col) => col.name)).size <
