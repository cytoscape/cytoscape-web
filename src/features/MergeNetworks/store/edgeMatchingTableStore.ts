        ...
        networkRecords[net1]?.edgeTable.columns.forEach(col => {
            if (!sharedColsRecord[net1]?.has(col.name)) {
                ...
                networkIds.slice(index1 + 1).forEach(net2 => {
                    if (networkRecords[net2]?.edgeTable.columns.some(nc => nc.name === col.name)) {
                        ...
                        const colType = networkRecords[net2]?.edgeTable.columns.find(nc => nc.name === col.name)?.type;
...
const removeNetworks = (state: EdgeMatchingTableStore, networkIds: IdType[]) => {
...
const useEdgeMatchingTableStore = create(immer<EdgeMatchingTableStore>((set) => ({
    rows: [],
    networkIds: new Set(),
