...
type EdgeMatchingTableStore = EdgeMatchingTableState & EdgeMatchingTableActions

const addNetworks = (
  state: EdgeMatchingTableStore,
  networkIds: IdType[],
  networkRecords: Record<IdType, NetworkRecord>,
  matchingCols: Record<string, Column>,
) => {
  ...
  state.rows = state.rows.map((row, id) => {
    ...
    for (const netId of networkIds) {
      const netCols = networkRecords[netId].edgeTable.columns
      if (netCols.some((nc) => nc.name === row.mergedNetwork)) {
        ...
        row.typeRecord[netId] =
          netCols.find((nc) => nc.name === row.mergedNetwork)?.type || 'None'
        typeCheck = true
  ...
  networkIds.forEach((net1, index1) => {
    ...
    networkRecords[net1]?.edgeTable.columns.forEach((col) => {
      if (!sharedColsRecord[net1]?.has(col.name)) {
        ...
        originalNetworkIds
          .concat(networkIds.slice(0, index1) ?? [])
          .forEach((net) => {
            matchCols[net] = 'None'
            typeRecord[net] = 'None'
          })
        networkIds.slice(index1 + 1).forEach((net2) => {
          if (
            networkRecords[net2]?.edgeTable.columns.some(
              (nc) => nc.name === col.name,
            )
          ) {
            ...
            const colType = networkRecords[net2]?.edgeTable.columns.find(
              (nc) => nc.name === col.name,
            )?.type
        ...
        const mergedNetworkName = generateUniqueName(
          mergedNetworkNames,
          col.name,
        )
...
const useEdgeMatchingTableStore = create(
  immer<EdgeMatchingTableStore>((set) => ({
    rows: [],
    networkIds: new Set(),
    setAllRows: (newRows) =>
      set((state) => {
    ...
    removeNetworksFromTable: (networkIds) =>
      set((state) => {
        removeNetworks(state, networkIds)
      }),
  })),
)

export default useEdgeMatchingTableStore
