...
const addNetworks = (
  ...
  if (state.rows.length > 0) {
        ...
        for (const netId of networkIds) {
          ...
          if (netCols.some((nc) => nc.name === row.mergedNetwork)) {
            ...
            row.typeRecord[netId] =
              netCols.find((nc) => nc.name === row.mergedNetwork)?.type ||
              'None'
            typeCheck = true
    ...
    for (const netId of networkIds) {
      ...
      const colType =
        (matchingCols[netId]?.type as ValueTypeName | 'None') || 'None'
    ...
    networkRecords[net1]?.nodeTable.columns.forEach((col) => {
      if (!sharedColsRecord[net1]?.has(col.name)) {
        ...
        networkIds.slice(index1 + 1).forEach((net2) => {
          if (
            networkRecords[net2]?.nodeTable.columns.some(
              (nc) => nc.name === col.name,
            )
          ) {
            ...
            const colType = networkRecords[net2]?.nodeTable.columns.find(
              (nc) => nc.name === col.name,
            )?.type
  ...
  if (checkFirstRow) {
    state.rows[0].mergedNetwork = generateUniqueName(
      mergedNetworkNames,
      state.rows[0].mergedNetwork,
    )
  }
}
...
const useNodeMatchingTableStore = create(
  immer<NodeMatchingTableStore>((set) => ({
    rows: [],
    networkIds: new Set(),
    setAllRows: (newRows) =>
      set((state) => {
