    ...
    networkRecords[net1]?.netTable?.columns.forEach((col) => {
      if (!sharedColsRecord[net1]?.has(col.name)) {
        ...
        networkIds.slice(index1 + 1).forEach((net2) => {
          if (
            networkRecords[net2]?.netTable?.columns.some(
              (nc) => nc.name === col.name,
            )
          ) {
            ...
            const colType = networkRecords[net2]?.netTable?.columns.find(
              (nc) => nc.name === col.name,
            )?.type
...
const useNetMatchingTableStore = create(
  immer<NetMatchingTableStore>((set) => ({
    rows: [],
    networkIds: new Set(),
    setAllRows: (newRows) =>
      set((state) => {
