...
import {
  ...
  mergeAttributes,
  preprocess,
} from '../../utils/attributesOperationsUtil'
import { removePair } from '../../utils/mergeNetworkUtil'
import { NetworkRecord, Pair } from '../DataInterfaceForMerge'
import { MatchingTable } from '../MatchingTable'
import {
  getMergedAttributes,
  getReversedMergedAttMap,
} from './MatchingTableImpl'
...
export function differenceMerge(
  ...
  if (fromNetworks.length !== 2) {
    throw new Error(
      'Difference merge can only be operated between two networks.',
    )
  }
  ...
  if (
    duplicateAttName(nodeMergedAttributes) ||
    duplicateAttName(edgeMergedAttributes)
  ) {
    throw new Error('Duplicate merged attribute names found')
  }
  const reversedAttMap = getReversedMergedAttMap(nodeAttributeMapping)
  const mergedAttCol: Column = {
    name: nodeMergedAttributes[0].name,
  ...
  function getIdentifiableFromEdge(
    edgeObj: Edge,
    netId: IdType,
    colName: string,
  ): [SingleValueType, SingleValueType, string] {
  ...
  networkRecords[baseNetId].network.nodes.forEach((nodeObj) => {
    ...
    if (mergeWithinNetwork && matchingAttributeMap.has(attributeMapKey)) {
      ...
      if (matchedNodeId !== undefined) {
        initialNodeRows[matchedNodeId] = mergeAttributes(
          //merge within the network
          initialNodeRows[matchedNodeId],
          castAttributes(nodeEntry, baseNetId, nodeAttributeMapping),
        )
      ...
      initialNodeRows[newNodeId] = addMergedAtt(
        castAttributes(nodeEntry, baseNetId, nodeAttributeMapping),
        nodeEntry[reversedAttMap.get(baseNetId) as string],
        mergedAttCol,
      )
  ...
  if (strictRemoveMode) {
    // subtract the node as long as it exists in the second network
    ...
    // Only remove nodes if all their edges are being subtracted, too
    networkRecords[baseNetId].network.edges.forEach((edgeObj) => {
      const [sourceNodeKey, targetNodeKey, edgeInteraction] =
        getIdentifiableFromEdge(edgeObj, baseNetId, baseMatchingColName)
      if (sourceNode2EdgeMap.has(sourceNodeKey)) {
        sourceNode2EdgeMap
          .get(sourceNodeKey)
          ?.push([`${targetNodeKey}-${edgeInteraction}`, edgeObj.id] as Pair<
            string,
            IdType
          >)
      } else {
        sourceNode2EdgeMap.set(sourceNodeKey, [
          [`${targetNodeKey}-${edgeInteraction}`, edgeObj.id] as Pair<
            string,
            IdType
          >,
        ])
      ...
      if (targetNode2EdgeMap.has(targetNodeKey)) {
        targetNode2EdgeMap
          .get(targetNodeKey)
          ?.push([`${sourceNodeKey}-${edgeInteraction}`, edgeObj.id] as Pair<
            string,
            IdType
          >)
      } else {
        targetNode2EdgeMap.set(targetNodeKey, [
          [`${sourceNodeKey}-${edgeInteraction}`, edgeObj.id] as Pair<
            string,
            IdType
          >,
        ])
    ...
    networkRecords[secondNetId].network.edges.forEach((edgeObj) => {
      const [sourceNodeKey, targetNodeKey, edgeInteraction] =
        getIdentifiableFromEdge(edgeObj, secondNetId, secMatchingColName)
      if (sourceNode2EdgeMap.has(sourceNodeKey)) {
        // remove the edge interaction from the list
        removePair(
          sourceNode2EdgeMap.get(sourceNodeKey) as Pair<string, IdType>[],
          `${targetNodeKey}-${edgeInteraction}`,
        )
        edgeToRemove.add(`${sourceNodeKey}-${targetNodeKey}-${edgeInteraction}`)
      }
      if (targetNode2EdgeMap.has(targetNodeKey)) {
        removePair(
          targetNode2EdgeMap.get(targetNodeKey) as Pair<string, IdType>[],
          `${sourceNodeKey}-${edgeInteraction}`,
        )
        edgeToRemove.add(`${sourceNodeKey}-${targetNodeKey}-${edgeInteraction}`)
      }
    })
    ...
    for (const nodeId of matchedNodes) {
      const nodeKey = getKeybyAttribute(
        initialNodeRows[nodeId][baseMatchingColName],
      )
      if (
        (sourceNode2EdgeMap.get(nodeKey)?.length ?? 0) > 0 ||
        (targetNode2EdgeMap.get(nodeKey)?.length ?? 0) > 0
      ) {
        NetworkFn.addNode(mergedNetwork, nodeId)
  ...
  networkRecords[baseNetId].network.edges.forEach((edgeObj) => {
    ...
    if (
      sourceNodeId &&
      targetNodeId &&
      unmatchedNodes.has(sourceNodeId) &&
      unmatchedNodes.has(targetNodeId) &&
      (strictRemoveMode ||
        !edgeToRemove.has(
          `${sourceNodeKey}-${targetNodeKey}-${edgeInteraction}`,
        ))
    ) {
      const newEdgeId = `e${globalEdgeId++}`
      initialEdgeRows[newEdgeId] = castAttributes(
        edgeEntry,
        baseNetId,
