  ...
  matchingTableRows.slice(3).forEach((row) => {
    for (const netId of fromNetworks) {
      if (
        ...
        const oriPropt = netSummaries[netId]?.properties.find(
          (property) => property.predicateString === oriProptName,
        )
        if (!mergedProperties[`${row.id}`] && oriPropt) {
          mergedProperties[`${row.id}`] = mergeProperty(
            row.mergedNetwork,
            cloneDeep(oriPropt),
          )
        } else if (oriPropt) {
          mergedProperties[`${row.id}`] = mergeProperty(
            row.mergedNetwork,
            cloneDeep(mergedProperties[`${row.id}`]),
            cloneDeep(oriPropt),
          )
        }
      }
...
function mergeProperty(
  ...
  if (propt1.dataType === propt2.dataType) {
    ...
    if (Array.isArray(value1) && Array.isArray(value2)) {
      // if both properties are list type
