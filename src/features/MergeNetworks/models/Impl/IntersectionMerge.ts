...
import { attributeValueMatcher } from '../../utils/attributesOperationsUtil'
import {
  addMergedAtt,
  castAttributes,
  duplicateAttName,
...
export function intersectionMerge(
  fromNetworks: IdType[],
  toNetworkId: IdType,
  ...
  if (fromNetworks.length < 2) {
    throw new Error(
      'Intersection merge must be operated between two or more networks.',
    )
  }
  ...
  networkRecords[baseNetworkId]?.network.nodes.forEach((nodeObj) => {
    ...
    const attributeMapKey = getKeybyAttribute(
      entry[matchingAttribute[baseNetworkId].name],
    )
    ...
    if (mergeWithinNetwork && matchingAttributeMap.has(attributeMapKey)) {
      ...
      if (matchedNodeId !== undefined) {
        initialNodeRows[matchedNodeId] = mergeAttributes(
          ...
          castAttributes(entry, baseNetworkId, nodeAttributeMapping),
        )
      ...
      initialNodeRows[newNodeId] = addMergedAtt(
        castAttributes(entry, baseNetworkId, nodeAttributeMapping),
        entry[reversedAttMap.get(baseNetworkId) as string],
        mergedAttCol,
      )
    }
  })
  ...
  for (const netToMerge of fromNetworks.slice(1)) {
    ...
    for (const nodeObj of nodeLst) {
      ...
      const matchedNodeId = attributeValueMatcher(
        nodeRecord[matchingAttribute[netToMerge].name],
        matchingAttributeMap,
      )
      if (matchedNodeId) {
    ...
    for (const [mergedNodeId, newNodeId] of matchedNodeIds) {
      ...
      const nodeRecord =
        networkRecords[netToMerge].nodeTable.rows.get(newNodeId)
      const originalRow = initialNodeRows[mergedNodeId]
      if (originalRow === undefined)
        throw new Error('Node not found in the node table')
      const castedRecord = castAttributes(
        nodeRecord,
        netToMerge,
  ...
  networkRecords[baseNetworkId]?.network.edges.forEach((oriEdge) => {
    ...
    if (
      intersectedNodeIds.has(newSourceId) &&
      intersectedNodeIds.has(newTargetId)
    ) {
      const edgeInteraction = oriEntry['interaction'] ?? ''
      const mergedEdgeIds = edgeMap.get(
        `${newSourceId}-${newTargetId}-${edgeInteraction}`,
      )
      if (mergeWithinNetwork && mergedEdgeIds) {
        const originalRow = initialEdgeRows[mergedEdgeIds[0]]
        initialEdgeRows[mergedEdgeIds[0]] = mergeAttributes(
          originalRow,
          castAttributes(oriEntry, baseNetworkId, edgeAttributeMapping, false),
        )
      } else {
        ...
        if (mergedEdgeIds) {
          ...
          edgeMap.set(`${newSourceId}-${newTargetId}-${edgeInteraction}`, [
            newEdgeId,
          ])
        }
      }
  ...
  for (const netToMerge of fromNetworks.slice(1)) {
    ...
    for (const edgeObj of edgeLst) {
      ...
      const edgeRecord =
        networkRecords[netToMerge].edgeTable.rows.get(oriEdgeId)
      ...
      if (mergedEdgeIds) {
        ...
        initialEdgeRows[mergedEdgeIds[0]] = mergeAttributes(
          originalRow,
          castAttributes(edgeRecord, netToMerge, edgeAttributeMapping, false),
        )
        continue
      }
      if (
        mergeOnlyNodes &&
        intersectedNodeIds.has(sourceId as string) &&
        intersectedNodeIds.has(targetId as string)
      ) {
