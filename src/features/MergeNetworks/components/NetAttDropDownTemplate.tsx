...
export const NetAttDropDownTemplate = React.memo(
  ({
    networkRecords,
    rowData,
    rowIndex,
    ...
    const networkOptions =
      (type === TableView.node && rowData.id === 0) ||
      (type === TableView.network && rowData.id < 3)
        ? columns.map((nc) => ({ label: nc.name, value: nc.name }))
        : [
            ...columns.map((nc) => ({ label: nc.name, value: nc.name })),
            emptyOption,
          ]
    ...
    const onDropdownChange = (
      ...
      const newType =
        columns.find((col) => col.name === newName)?.type || 'None'
      const newNameRecord = { ...rowData.nameRecord, [field]: newName }
      const newTypeRecord: Record<string, 'None' | ValueTypeName> = {
        ...rowData.typeRecord,
        [field]: newType,
      }
