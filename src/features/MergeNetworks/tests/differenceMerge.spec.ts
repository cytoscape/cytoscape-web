...
describe('intersection merge', () => {
  ...
  let networkRecords: Record<
    IdType,
    { network: Network; nodeTable: Table; edgeTable: Table }
  >
  ...
  beforeEach(() => {
    ...
    nodeTableRows1 = new Map(
      [...nodeLst1.entries()].map(([i, node]) => [
        node.id,
        { name: `node ${i + 1}` },
      ]),
    )
    nodeTableRows2 = new Map(
      [...nodeLst2.entries()].map(([i, node]) => [
        node.id,
        { name: `node ${i + 1}` },
      ]),
    )

    edgeTableRows1 = new Map(
      edgeLst1.map((edge) => [
        edge.id,
        { name: `edge ${edge.id?.slice(1)}`, interaction: 'a' },
      ]),
    )
    edgeTableRows2 = new Map(
      edgeLst2.map((edge) => [
        edge.id,
        {
          name: `edge ${edge.id?.slice(1)}`,
          interaction: edge.id === 'e6' ? 'b' : 'a',
        },
      ]),
    )
