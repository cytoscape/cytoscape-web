...
export const networkApi: NetworkApi = {
  ...
  deleteNetwork(networkId, _options) {
    try {
      ...
      // useDeleteCyNetwork (REVIEW.md A4). It also owns the
      // currentNetworkId invariant: the pointer is repaired whenever the
      // deleted network was current, regardless of the navigate option
      // (R2-13 — previously navigate:false left it dangling).
  ...
  deleteAllNetworks() {
    try {
      // Single source of truth for the cascade (REVIEW.md A4)
      deleteAllNetworksFromAllStores()
