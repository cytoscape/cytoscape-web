...
const prepareChange = async (
  ...
  switch (table) {
    ...
    case 'cyVisualStyles': {
      ...
      // Hydrate the whole style set, not just the active style: a peer that
      // created, renamed or switched a named style changed only the set
      // metadata, and add() keeps the local set when no set is passed.
      const styleSet = await getVisualStyleSetFromDb(key)
      const style = styleSet?.styles[styleSet.activeStyleId]?.visualStyle
      if (!styleSet || !style) {
        return null
      }
      return () => useVisualStyleStore.getState().add(key, style, styleSet)
    }
