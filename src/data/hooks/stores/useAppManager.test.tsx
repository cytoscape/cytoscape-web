...
jest.mock('../../../app-api/core', () => ({ CyWebApi: {} }))

jest.mock('../../db', () => ({
  ...jest.requireActual('../../db'),
  getAppSettingFromDb: jest.fn().mockResolvedValue(undefined),
...
jest.mock('../../../features/AppManager/manifest/obtainCatalogEntries', () => ({
  obtainCatalogEntries: jest.fn().mockResolvedValue([]),
}))

jest.mock('../../../features/AppManager/install/migrateLegacyApps', () => ({
  migrateLegacyApps: jest.fn().mockResolvedValue(undefined),
}))

jest.mock('../../../features/AppManager/install/installGate', () => ({
  isAllowedOrigin: jest.fn(() => true),
  isHostCompatible: jest.fn(() => true),
}))

jest.mock('./appLifecycle', () => ({
  mountApp: jest.fn().mockResolvedValue(undefined),
  unmountApp: jest.fn().mockResolvedValue(undefined),
  unmountAllApps: jest.fn().mockResolvedValue(undefined),
}))

jest.mock('../../../features/AppManager/loader/loadRemoteApp', () => ({
  loadRemoteApp: jest.fn(),
}))

const { loadRemoteApp } = jest.requireMock(
  '../../../features/AppManager/loader/loadRemoteApp',
)

const mockIsAllowedOrigin = isAllowedOrigin as jest.Mock
const mockIsHostCompatible = isHostCompatible as jest.Mock
const mockMigrate = migrateLegacyApps as jest.Mock
...
describe('useAppManager — install / uninstall', () => {
  ...
  describe('status reconciliation', () => {
    it('creates a source:manifest record when a manifest app is activated', async () => {
      ...
      loadRemoteApp.mockImplementation(
