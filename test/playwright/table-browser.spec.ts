...
test.describe('Table Browser', () => {
  test.describe('1. Basic Rendering & Tabs', () => {
    ...
    test('1.2 Tab Switching', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="table-browser-node-editor"]'),
      ).toBeVisible()

      // Switch to Edges tab
      await page.click('[data-testid="table-browser-edges-tab"]')
      await expect(
        page.locator('[data-testid="table-browser-edge-editor"]'),
      ).toBeVisible()
      ...
      await expect(
        page
          .locator('[data-testid="network-info-panel"]')
          .getByText('Test Network'),
      ).toBeVisible()
    })
  })
  ...
  test.describe('3. Column Operations', () => {
    test('3.1 Insert Column', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="create-table-column-dialog"]'),
      ).toBeVisible()

      // Fill in column name
      await page.fill(
        '[data-testid="create-table-column-name-input"] input',
        'Test Column',
      )

      // Default type is String, default value empty. Fill a default value
      await page.fill(
        '[data-testid="create-table-column-default-value-input"] input',
        'test_val',
      )
      ...
      expect(nodeTableResult).toEqual(
      ...
      expect(
        tableData.columns.find((c: any) => c.name === 'Test Column'),
      ).toBeDefined()
