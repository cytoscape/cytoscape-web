// Shared run-and-capture tail for the benchmark suites. Ported from the
// cytoscape.js v4 harness.
//
...
import { N } from './bench-size.mjs' // not fixture.mjs: that one loads the app
...
function slimStats(stats) {
  ...
  if (Array.isArray(stats?.samples)) {
    out.samples = stats.samples.length
  }
...
/**
 * The same tail for **manually timed** suites.
 *
 * Suites that time one shot per row rather than sampling through mitata —
 * deliberately, because their rows mutate or are one-offs — have no mitata
 * results to hand over. This turns their rows into the same job shape, with
 * every percentile equal to the single measurement (`oneShotStats`).
 * Rows with samples === 1 are "unscreened" in cross-run comparisons.
 *
...
export function finishManualRun(suite, groups) {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      groups: groups.map((g) => ({
        name: g.name,
        benches: g.benches.map((b) => ({
          name: b.name,
          stats: oneShotStats(b.ms),
        })),
      })),
    }),
  )
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      groups: groups.filter(
        (g) => g.benches.length > 0 && typeof g.name === 'string',
      ),
    }),
  )
}
