// Ported from the cytoscape.js v4 benchmark harness. 'Round N' references
// cite that project's development log, where these designs were measured
// and argued; the reasoning transfers, the rounds themselves are theirs.
 ...
 * builders (`fixture.mjs` and the generator it wraps), the process
 * environment (`bench-env.mjs`), the run size (`bench-size.mjs`), the
 * sampling and JSON tail (`bench-run.mjs`), and the one-shot/array stat
 * shapers (`render-stats.mjs`).  A change to any of them invalidates
 * comparison across it for all suites, which is why they are folded into
 * every hash rather than tracked per suite.
 */
export const SHARED_HARNESS = [
  'benchmark/bench-run.mjs',
  'benchmark/bench-size.mjs',
  'benchmark/bench-env.mjs',
  'benchmark/fixture.mjs',
  'benchmark/render-stats.mjs',
  // fixture.mjs builds its documents through the shared test-fixture
  // generator, which sits outside benchmark/ — its `../` import is not
  // followed by the traversal, but the generator's output IS the operand
  // every suite measures, so a change to it must break every epoch.
  'scripts/generate-test-fixtures/generate-cx2.ts',
]

/**
 * Extra inputs for suites whose harness is not one file — a suite that
 * drives a page or an external process lists what it drives here.
 */
const EXTRA_INPUTS = {}

/**
 * Modules a suite imports that render or describe a run rather than measure
 * it.  A suite that writes its own report or `meta` block (browser-bench)
 * would otherwise fold the report's HTML into its epoch, so a cosmetic report
 * change would break every browser epoch for no measurement reason — a false
 * break is as damaging as a missed one, because it teaches the reader to
 * ignore breaks.
 */
 ...
 */
const DYNAMIC_SUITES = [{ prefix: 'browser — ', file: 'browser-bench.mjs' }]
...
export const EQUIVALENT_HARNESSES = [
  // (none yet — the ledger starts empty, deliberately: the first entry
  // should be a real cosmetic change, not a placeholder nobody revisits)
]
